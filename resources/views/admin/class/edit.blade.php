...
<div class="class-form-container">
    <div class="class-form-box">
        ...
        <form action="{{ route('class.update', $class->id) }}" method="POST" class="class-form">
            @csrf
            @method('PUT')
            ...
            <div class="form-group">
                ...
                <input type="text" name="genre" id="genre" class="form-input" 
                       value="{{ old('genre', $class->genre) }}" required>
                @error('genre')
                    <span class="form-error">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <textarea name="description" id="description" class="form-textarea">{{ old('description', $class->description) }}</textarea>
                @error('description')
                    <span class="form-error">{{ $message }}</span>
                @enderror
            </div>
                ...
                <select name="instructor_id" id="instructor_id" class="form-select">
                    ...
                    @foreach ($instructors as $instructor)
                        <option value="{{ $instructor->id }}" 
                                {{ old('instructor_id', $class->instructors_id) == $instructor->id ? 'selected' : '' }}>
            ...
            <div class="form-group">
                ...
                <input type="number" name="availability" id="availability" class="form-input" 
                       value="{{ old('availability', $class->availability) }}" min="0" required>
                @error('availability')
                    <span class="form-error">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="datetime-local" name="time_start" id="time_start" class="form-input" 
                       value="{{ old('time_start', date('Y-m-d\TH:i', strtotime($class->time_start))) }}" required>
                @error('time_start')
                    <span class="form-error">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="datetime-local" name="time_ends" id="time_ends" class="form-input" 
                       value="{{ old('time_ends', date('Y-m-d\TH:i', strtotime($class->time_ends))) }}" required>
                @error('time_ends')
                    <span class="form-error">{{ $message }}</span>
                @enderror
            </div>
